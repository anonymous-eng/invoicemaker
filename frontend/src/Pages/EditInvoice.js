...
import { getInvoice, updateInvoice } from '../actions/invoicesActions';
...
const EditInvoice = () => {
  ...
  const { loading, clients, success } = clientList; 

  const invoiceList = useSelector((state) => state.invoiceList);
  const { invoice, loading: invoiceLoading } = invoiceList; 
  ...
  return (
    ...
    { !loading && invoice && <div className='bg-slate-100 m-4 shadow-2xl rounded-lg p-4 flex gap-4 flex-col'>
      ...
      <div className='p-2 flex flex-col gap-4 md:flex-row md:justify-between'>
        <div>
          ...
          : <div className='relative flex flex-col md:flex-row gap-4'>
            <button onClick={() => setOpen((prev) => !prev)} className='w-fit tracking-wider bg-[#0F959C] shadow-lg p-2 border-4 border-transparent active:border-black duration-100 active:text-black rounded-lg font-semibold text-white'>New Customer</button>
            <button onClick={() => setIsOpen((prev) => !prev)} className='flex items-center justify-between tracking-wider border-4 border-transparent active:border-black duration-100 active:text-black w-fit bg-[#0F959C] shadow-lg p-2 rounded-lg font-semibold text-white' >
              Existing Customer
        ...
        <div className='flex flex-col p-2 items-end'>
          ...
          <h2 className={`font-bold text-white p-1 rounded-lg w-fit tracking-wide font-serif text-lg mb-2 ${bgColorClass}`}>{status}</h2>
